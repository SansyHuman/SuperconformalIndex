...
decompositions and final singlet coefficients. Sage supplies input/representation
data, the dual-label convention and the exact returned polynomial ring.

This revision updates the cache, projection algorithm, helper descriptions and
validation. The unrevised index convention, matter characters, FORM expansion,
JSON examples and interpretation are retained. Earlier validation is dated
explicitly. The companion implementation reference provides package-wide context.
...
8 The SQLite character and singlet cache & \pageref{sec:cache}\\
...
\clearpage
\section{The SQLite character and singlet cache}\label{sec:cache}
...
FORM still performs the fugacity expansion on every run.
...
\code{_parse_form_output} & Converts each exact FORM monomial to a \code{FormTerm};
rational coefficients become Python \code{Fraction} objects.\\
\code{get_decomposition},\newline\code{get_decompositions} & Retrieve or construct
Eq.~(21); batch missing prerequisites by tensor-factor count.\\
...
\end{lstlisting}
The default cache is \code{char_decomposition_cache.db} at the project root.
...
arguments are \code{database_path=} and \code{cache_directory=}.

For example, an unlimited single-worker calculation through $t^{18}$ is
\begin{lstlisting}[language=Python]
result = calculate_index(data, 18, processes=1, timeout=None,
                         database_path="/tmp/fresh_index_cache.db")
\end{lstlisting}
Use a previously nonexistent database for a cold-cache timing. \code{timeout=None}
...
Timings are machine-dependent. They illustrate that the persistent decomposition
cache removes repeated LiE work but not FORM's per-run series expansion.
...
preceded promotion to the implementation now committed as \code{0a402ec}.
...
\clearpage
\subsection{Measured speedups and unlimited A32 profile}
