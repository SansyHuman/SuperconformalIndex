...
\fancyhead[R]{\small\sffamily\color{muted}9 September 2026}
...
\vspace{11mm}
\begin{note}{Database update: schema version 6}
This update adds a database relationship diagram, a catalog of all ten tables
and their keys, and the full/half-hyper normalization rules. It documents the
$5\to6$ migration, conditional legacy-JSON cleanup, and the separate migration
still needed for old realization hashes. The validation snapshot is refreshed.
\end{note}
\vfill
\begin{tabularx}{\textwidth}{@{}p{31mm}Y@{}}
Snapshot & Base reference: 8 September 2026. Database and validation update:
9 September 2026; \code{662ad31} plus working-tree changes.\\
...
Status & Database structure checked against the current source; other package
...
Editing scope & Documentation only. The legacy index-background PDF is retained;
this consolidated reference supersedes it.\\
\end{tabularx}
\end{titlepage}
...
\code{index} & FORM expansion, LiE gauge projection and cache; Coulomb PE/PL and
...
\code{_parse_form_output} converts rational coefficients, fugacity exponents and
character powers into \code{FormTerm} records. The numerical coefficients remain
...
\code{CharacterDecompositionCache} uses LiE Adams and tensor operations to obtain
the coefficients in \eqref{eq:adams}. Requests are keyed by Cartan type, Dynkin
labels and the complete Adams-power tuple $(n_1,n_2,\ldots)$. The weighted order
\begin{equation}
 w=\sum_{k\geq1}k n_k
 \label{eq:adams-order}
\end{equation}
groups entries into files; it is not alone a unique cache key. For example,
\code{A1/A1_dynkin_1_adams_order_2.json} can contain different tuples of weight two.
Dependency reuse, in-memory lookups and persistent JSON avoid repeating LiE
work. Batch requests can use worker processes; locks and atomic replacement
preserve entries sharing a cache file.

For each gauge factor, tensor products of the resulting simple characters are
decomposed and only the trivial-representation coefficient is retained:
\begin{equation}
 ...
 \label{eq:haar}
\end{equation}
\code{_project_terms} performs this factorwise calculation without expanding all
gauge torus monomials in FORM. $B_r,D_r$ continue to mean Spin representations,
not a restriction to representations descending to $SO$ quotients.
...
transactions; actual index equality through $t^6$.\\\bottomrule
\end{tabularx}}

\subsection{Validation snapshot}
The recorded 9 September 2026 run after the database fix completed in 9.507 s:
\begin{center}
{\large\sffamily\color{accent}134 tests run: 132 passed, 2 skipped.}
\end{center}
The two skipped tests require live MySQL. Database unit tests use recording/mocked
connections; a backend regression compares actual FORM/LiE indices for full/half
$SU(2)$ fundamentals and $SU(2)^3$ trifundamentals. Other tests cover Tits' reality
formula, irrep/theory enumeration and index/Coulomb calculations. This recorded
run does not validate live migration or arbitrary high-order performance.
...
low-level APIs accept executable paths, timeouts and cache directories.
...
Software documentation was consulted on 8 September 2026. This bibliography
