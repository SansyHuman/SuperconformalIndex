...
still needed for old realization hashes. The database validation snapshot is retained below.
\end{note}
\vfill
\begin{tabularx}{\textwidth}{@{}p{31mm}Y@{}}
Snapshot & Base reference: 8 September 2026. Database and validation update:
9 September 2026. Index/cache update: 10 September 2026; \code{0a402ec}.\\
...
Status & Index/cache algorithms and validation refreshed, including the unlimited
A32 profile. Database structure retains its version-6 update; unrelated package
...
Both caches use \code{sqlite3} and default to
...
\clearpage
\subsubsection{Validation, measured speedups and the remaining bottleneck}
The temporary candidate was compared against commit \code{a9b7009} before
promotion; the current implementation is committed in \code{0a402ec}.
...
The post-projection run on 9 September 2026 completed in 11.461 s:
\textbf{170 tests run, 168 passed, 2 skipped}. The cache tests cover persistence
and concurrent access; seven new \code{test_singlet_projection.py} regressions
cover signed pairings, independent weights, grouped/mixed products and parallel
execution. Detailed benchmarks appear in the revised index chapter.

\textbf{Earlier database snapshot (retained).} The recorded 9 September 2026 run after the database fix completed in 9.507 s:
\begin{center}
